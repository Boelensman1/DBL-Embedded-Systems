...
_pow1:               LOAD R5 1
                     RTS
_powR:               RTS
main:                LOAD R0 0                                     ;$temp=0
                     STOR R0 [GB +$outputs + HBRIDGE1]             ;_storeData($temp,'$outputs',HBRIDGE1)
                     STOR R0 [GB +$outputs + HBRIDGE0]             ;_storeData($temp,'$outputs',HBRIDGE0)
                     STOR R0 [GB +$outputs + LENSLAMPPOSITION]     ;_storeData($temp,'$outputs',LENSLAMPPOSITION)
                     STOR R0 [GB +$outputs + LENSLAMPSORTER]       ;_storeData($temp,'$outputs',LENSLAMPSORTER)
                     STOR R0 [GB +$outputs + LEDSTATEINDICATOR]    ;_storeData($temp,'$outputs',LEDSTATEINDICATOR)
                     STOR R0 [GB +$outputs + DISPLAY]              ;_storeData($temp,'$outputs',DISPLAY)
                     STOR R0 [GB +$outputs + CONVEYORBELT]         ;_storeData($temp,'$outputs',CONVEYORBELT)
                     STOR R0 [GB +$outputs + FEEDERENGINE]         ;_storeData($temp,'$outputs',FEEDERENGINE)
                     LOAD R1 0                                     ;$state=0
...
initial:             BRS timerManage                               ;timerManage($outputs)
...
calibrateSorter:     BRS timerManage                               ;timerManage($outputs)
...
resting:             BRS timerManage                               ;timerManage($outputs)
...
return2:             BRA resting                                   ;resting()
                     BRA main
                                                                   ...
                                                                   ;if ($startStop == 1) {
conditional2:        LOAD R0 12                                    ;$temp=12
                     STOR R0 [GB +$outputs + LENSLAMPPOSITION]     ;_storeData($temp,'$outputs',LENSLAMPPOSITION)
                     STOR R0 [GB +$outputs + LENSLAMPSORTER]       ;_storeData($temp,'$outputs',LENSLAMPSORTER)
                     LOAD R0 9                                     ;$temp=9
                     STOR R0 [GB +$outputs + CONVEYORBELT]         ;_storeData($temp,'$outputs',CONVEYORBELT)
...
running:             BRS timerManage                               ;timerManage($outputs)
                     ...
                     BEQ conditional3
return3:             CMP R2 1                                      ;if ($position == 1) {
                     BEQ conditional4
return4:             BRA running                                   ;running()
                     BRA main
                     
                                                                   ;if ($startStop == 1) {
conditional3:        LOAD R0 0                                     ;$temp=0
                     STOR R0 [GB +$outputs + FEEDERENGINE]         ;_storeData($temp,'$outputs',FEEDERENGINE)
...
runningWait:         BRS timerManage                               ;timerManage($outputs)
                     PUSH R3                                       ;$position = _getButtonPressed(7)
                     LOAD R3 7
                     BRS _pressed
                     PULL R4
                     ...
                     CMP R3 1                                      ;if ($startStop == 1) {
                     BEQ conditional5
return5:             CMP R0 1                                      ;if ($position == 1) {
                     BEQ conditional6
...
motorUp:             BRS timerManage                               ;timerManage($outputs)
...
whiteWait:           BRS timerManage                               ;timerManage($outputs)
                     PUSH R3                                       ;$startStop = _getButtonPressed(0)
                     LOAD R3 0
                     BRS _pressed
                     PULL R4
                     ...
                     BEQ conditional10
return10:            CMP R0 1                                      ;if ($startStop == 1) {
                     BEQ conditional11
return11:            ADD R1 1                                      ;$sleep+=1
                     BRA whiteWait                                 ;whiteWait()
...
whiteWaitTimer:      BRS timerManage                               ;timerManage($outputs)
                     BRA whiteWaitStop                             ;whiteWaitStop()
                     
whiteWaitStop:       BRS timerManage                               ;timerManage($outputs)
...
motorDownStop:       BRS timerManage                               ;timerManage($outputs)
                     CMP R1 TIMEMOTORDOWN                          ;if ($sleep == TIMEMOTORDOWN ) {
                     BEQ conditional13
...
runningStop:         BRS timerManage                               ;timerManage($outputs)
                     PUSH R3                                       ;$colour = _getButtonPressed(6)
                     LOAD R3 6
                     BRS _pressed
                     PULL R4
...
motorUpStop:         BRS timerManage                               ;timerManage($outputs)
                     PUSH R3                                       ;$push = _getButtonPressed(5)
                     LOAD R3 5
                     BRS _pressed
                     PULL R4
...
motorDown:           BRS timerManage                               ;timerManage($outputs)
                     PUSH R3                                       ;$startStop = _getButtonPressed(0)
                     LOAD R3 0
                     BRS _pressed
                     PULL R4
                     ...
                     CMP R1 TIMEMOTORDOWN                          ;if ($sleep == TIMEMOTORDOWN ) {
                     BEQ conditional16
return16:            CMP R0 1                                      ;if ($startStop == 1) {
                     BEQ conditional17
...
motorDownTimer:      BRS timerManage                               ;timerManage($outputs)
                     BRA motorDownStop                             ;motorDownStop()
                     
motorUpTimer:        BRS timerManage                               ;timerManage($outputs)
                     BRA motorUpStop                               ;motorUpStop()
                     
runningTimerReset:   BRS timerManage                               ;timerManage($outputs)
                     BRA runningWait                               ;runningWait()
                     
runningTimer:        BRS timerManage                               ;timerManage($outputs)
                     BRA runningStop                               ;runningStop()
